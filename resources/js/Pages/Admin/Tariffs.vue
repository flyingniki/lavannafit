<template>
  <AdminLayout title="Тарифы">
    <div class="flex justify-between items-center mb-6">
      <p class="text-sm text-gray-500">Управляйте тарифами, которые отображаются на сайте.</p>
      <button @click="openCreate" class="btn-primary">+ Добавить тариф</button>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
      <table class="w-full text-sm">
        <thead class="bg-gray-50 border-b border-gray-100">
          <tr>
            <th class="text-left px-5 py-3 font-medium text-gray-500">Название</th>
            <th class="text-left px-5 py-3 font-medium text-gray-500">Цена</th>
            <th class="text-left px-5 py-3 font-medium text-gray-500">Активен</th>
            <th class="text-left px-5 py-3 font-medium text-gray-500">Порядок</th>
            <th class="px-5 py-3"></th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-50">
          <tr v-for="tariff in tariffs" :key="tariff.id" class="hover:bg-gray-50 transition-colors">
            <td class="px-5 py-4 font-medium text-gray-900">{{ tariff.name }}</td>
            <td class="px-5 py-4 text-gray-600">{{ tariff.price }} ₽ / {{ tariff.period }}</td>
            <td class="px-5 py-4">
              <span :class="tariff.is_active ? 'badge-green' : 'badge-gray'">
                {{ tariff.is_active ? 'Да' : 'Нет' }}
              </span>
            </td>
            <td class="px-5 py-4 text-gray-500">{{ tariff.sort_order }}</td>
            <td class="px-5 py-4 flex gap-2 justify-end">
              <button @click="openEdit(tariff)" class="btn-icon">✏️</button>
              <button @click="confirmDelete(tariff)" class="btn-icon text-red-500">🗑️</button>
            </td>
          </tr>
          <tr v-if="!tariffs.length">
            <td colspan="5" class="px-5 py-8 text-center text-gray-400">Тарифы не найдены</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <!-- Modal -->
    <Modal v-if="modal" :title="editingTariff ? 'Редактировать тариф' : 'Новый тариф'" @close="closeModal">
      <form @submit.prevent="submitForm" class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <Field label="Цена *">
            <input v-model="form.price" type="text" required class="input" placeholder="4 900" />
          </Field>
          <Field label="Период *">
            <input v-model="form.period" type="text" required class="input" placeholder="мес" />
          </Field>
        </div>
        <Field label="Описание">
          <textarea v-model="form.description" rows="2" class="input resize-none" />
        </Field>
        <Field label="Возможности (каждая с новой строки)">
          <textarea v-model="featuresText" rows="5" class="input resize-none font-mono text-xs" placeholder="Программа тренировок&#10;План питания" />
        </Field>
        <div class="grid grid-cols-2 gap-4">
          <Field label="Порядок">
...
const emptyForm = () => ({
  name: '', price: '', period: 'мес', description: '', features: [], sort_order: 0, is_active: true,
});
...
const featuresText = computed({
  ...
  set: (v) => { form.value.features = v.split('\n').map(s => s.trim()).filter(Boolean); },
});
...
function openEdit(tariff) {
  ...
  form.value = { ...tariff, features: tariff.features || [] };
  modal.value = true;
}
...
function submitForm() {
  ...
  const url = editingTariff.value
    ? `/admin/tariffs/${editingTariff.value.id}`
    : '/admin/tariffs';
  const method = editingTariff.value ? 'put' : 'post';
